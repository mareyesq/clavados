...
global $competencia, $evento, $descripcion, $fecha, $subtitulo,$subtitulo1,$logo,$organizador,$encabezado1,$header,$w, $logo2;
	...
	$this->ln(2);
	$this->Cell(0,10,'PUNTOS POR DEPORTISTA',0,0,'C');
	$this->SetLineWidth(.1);
	$this->SetFont('','B',8);
	$this->ln(8);
	$fill = false;
...
function FancyTable($data,$header, $w)
{
	...
	foreach ($data as $cla => $row) {
		...
		if ($puntos>0) {
			...
			$this->Cell($w[1],6,$nombre,0,0,'C',$fill);
			$this->Cell($w[2],6,strtoupper(substr($equipo,0,35)),0,0,'C',$fill);
			$this->Cell($w[3],6,$puntos,0,0,'C',$fill);
			$fill = !$fill;
			$this->Ln();
		}
	}
...
$header = array('Lugar','Deportista', 'Equipo','Puntos');
$w = array(30,60,80,10);
// Carga de datos
$pdf = new PDF();
