...
include("valida-planilla-mix.php");
...
if (is_null($mensaje)){
	...
	if ($verificado==1) {
		...
		header("Location: ..?op=php/cambio-planilla-mix.php&mensaje=$mensaje&ok=1");
		exit();
	}
	...
	if ($modificar==1) {
		...
		$mensaje=NULL;
		header("Location: ..?op=php/cambio-planilla-mix.php&ok=0");
		exit();
	}

	if ($guardar==0) {
		// está todo OK pero no seleccionó Guardar
		header("Location: ..?op=php/cambio-planilla-mix.php&ok=1");
		exit();
	}
	...
	for ($i=1; $i<=$tot_sal ; $i++) { 
		...
		if (in_array($i, $rondas)){
			...
			$consulta=
				"INSERT INTO planillad (planilla, ronda, salto, posicion, altura, grado_dif, abierto, salto2, posicion2";
			$consulta .= ") VALUES ($cod_planilla,$i,'$cod','$pos', $alt, $dif,'$abi'";
			if ($co2) 
				$consulta .= ", '$co2', '$po2'";
			else
				$consulta .= ", NULL, NULL";
...
header("Location: ..?op=php/cambio-planilla-mix.php&mensaje=$mensaje");
 ?>