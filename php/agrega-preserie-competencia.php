...
if (is_null($mensaje) and $agregar) {
	$conexion=conectarse();
	...
	$conexion->close();
	if (is_null($mensaje)){
		$mensaje="Se registro la serie  <b>$modalidad - $categoria</b> :)";
		include ("preserie-limpia-session.php");
...
header("Location: ..?op=php/alta-preserie-competencia.php&mensaje=$mensaje");
 ?>