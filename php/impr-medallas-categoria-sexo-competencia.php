...
global $competencia, $evento, $descripcion, $fecha, $subtitulo,$subtitulo1,$logo,$organizador,$encabezado1,$header1,$w1,$logo2;
	...
	$this->ln(2);
	$this->Cell(0,10,'RESUMEN DE MEDALLAS POR CATEGORIA Y SEXO',0,0,'C');
	$this->SetLineWidth(.1);
	$this->SetFont('','B',8);
	$this->ln(8);
	$fill = false;
...
function rompe1($sexo,$w,$header){
	$this->SetFont('','B',10);
	$this->Cell(0,8,'       '.strtoupper(utf8_decode($sexo)),0,0,'L');
	$this->SetFont('','',8);
	$this->ln(3);
...
while ($reg=$ejecutar_consulta_posiciones->fetch_assoc()){
	acumula_medallas_cat_sexo($reg["categoria"],$reg["nom_cat"],$reg["sexo"],$reg["lugar"],$reg["equipo"],$reg["nom_equipo"]);
}
//              Abierta
while ($reg=$ejecutar_consulta_abierta->fetch_assoc()){
	acumula_medallas_cat_sexo("AB","Abierta",$reg["sexo"],$reg["lugar"],$reg["equipo"],$reg["nom_equipo"]);
}
...
$data=array();
for ($i=0; $i < $n ; $i++) { 
	$data[]=$cat_sx[$i]."-".$oro[$i]."-".$plata[$i]."-".$bronce[$i]."-".$equipos[$i]."-".$categorias[$i]."-".$sexos[$i];
}
rsort($data);
