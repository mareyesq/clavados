...
$consulta_posiciones="SELECT p.categoria, p.sexo, p.lugar, p.equipo, q.equipo as nom_equipo, k.categoria as nom_cat
	FROM planillas 	as p
...
$consulta_abierta="SELECT p.sexo,  p.lugar_abierta as lugar, p.equipo, q.equipo as nom_equipo
	FROM planillas 	as p
	LEFT JOIN categorias 	as k 	on k.cod_categoria='AB'
	LEFT JOIN modalidades 	as m 	on m.cod_modalidad=p.modalidad
	LEFT JOIN competenciast	as t 	on t.competencia=p.competencia and t.puesto=p.lugar
	LEFT JOIN competenciasq as q 	on (q.competencia=p.competencia and q.nombre_corto=p.equipo)";
$consulta_abierta .= " WHERE p.competencia = $cod_competencia AND (p.categoria='AB' or p.part_abierta='*') and m.individual=1 and p.momento_termino is NOT NULL AND p.extraof_abierto IS NULL and p.lugar_abierta<4 
 	ORDER BY p.sexo, q.equipo, p.lugar_abierta";
$ejecutar_consulta_abierta = $conexion->query($consulta_abierta);

$consulta_sinc="SELECT p.categoria, p.sexo, p.lugar, p.equipo, q.equipo as nom_equipo, k.categoria as nom_cat
	FROM planillas 	as p
	LEFT JOIN categorias 	as k 	on k.cod_categoria=p.categoria
	LEFT JOIN modalidades 	as m 	on m.cod_modalidad=p.modalidad
	LEFT JOIN competenciasq as q 	on (q.competencia=p.competencia and q.nombre_corto=p.equipo)";
...
$num_regs=$ejecutar_consulta_sinc->num_rows;
...
include("impr-medallas-categoria-sexo-competencia.php");
$conexion->close();
