...
if (is_null($mensaje)){
	...
	if ($verificado==1) {
		...
		if (isset($menos_saltos))
			$mensaje=$menos_saltos.". Pero puede guardar la planilla";
		header("Location: ..?op=php/cambio-planilla.php&mensaje=$mensaje&ok=1");
		exit();
	}
	...
	if ($modificar==1) {
		...
		$mensaje=NULL;
		header("Location: ..?op=php/cambio-planilla.php&ok=0");
		exit();
	}

	if ($guardar==0 and !$btn_autorizar) {
		// está todo OK pero no seleccionó Guardar
		header("Location: ..?op=php/cambio-planilla.php&ok=1");
		exit();
	}
			...
			$consulta=
				"UPDATE planillad 
					SET salto='$cod', 
						...
						grado_dif=$dif,
						abierto='$abi'
					WHERE  planilla=$cod_planilla and ronda=$i";
		else
			$consulta=
				"INSERT INTO planillad (planilla, ronda, salto, posicion, altura, grado_dif, abierto) VALUES ($cod_planilla,$i,'$cod','$pos', $alt, $dif,'$abi')";
		$ejecutar_consulta=$conexion->query($consulta);
	...
	if ($ejecutar_consulta){
		while ($reg=$ejecutar_consulta_saltos->fetch_assoc()) {
			...
			$res=suma($cal,$sincronizado,$num_jueces);
			$suma=$res["suma"];
			$t_salto=$res["puntaje"];
...
header("Location: ..?op=php/cambio-planilla.php&mensaje=$mensaje");
 ?>