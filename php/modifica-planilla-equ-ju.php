...
$_SESSION["cod_sexo3"]=$cod_sexo3;
$_SESSION["cod_sexo4"]=$cod_sexo4;
...
$nacimiento3=isset($_POST["nacimiento3_hdn"])?$_POST["nacimiento3_hdn"]:NULL;
$nacimiento4=isset($_POST["nacimiento4_hdn"])?$_POST["nacimiento4_hdn"]:NULL;
$_SESSION["nacimiento"]=$nacimiento;
$_SESSION["nacimiento2"]=$nacimiento2;
$_SESSION["nacimiento3"]=$nacimiento3;
$_SESSION["nacimiento4"]=$nacimiento4;
...
include("valida-planilla-equ-ju.php");
...
if (is_null($mensaje)){
	...
	if ($verificado==1) {
		...
		header("Location: ..?op=php/cambio-planilla-equ-ju.php&mensaje=$mensaje&ok=1");
		exit();
	}
	...
	if ($modificar==1) {
		...
		$mensaje=NULL;
		header("Location: ..?op=php/cambio-planilla-equ-ju.php&ok=0");
		exit();
	}

	if ($guardar==0) {
		// está todo OK pero no seleccionó Guardar
		header("Location: ..?op=php/cambio-planilla-equ-ju.php&ok=1");
		exit();
	}
	...
	if (isset($cod_clavadista3))
		$consulta .= ", clavadista3=$cod_clavadista3 ";
	if (isset($cod_clavadista4))
		$consulta .= ", clavadista4=$cod_clavadista4 ";
	...
	for ($i=1; $i<=$tot_sal ; $i++) { 
		...
		if (in_array($i, $rondas)){
			...
			if ($eje2) 
				$consulta .= ", ejecutor2=$eje2";
			else
				$consulta .= ", ejecutor2=NULL";

			$consulta .= " WHERE  planilla=$cod_planilla and ronda=$i";
		}
		else{
			$consulta=
				"INSERT INTO planillad (planilla, ronda, salto, posicion, altura, grado_dif, ejecutor, ejecutor2";
...
header("Location: ..?op=php/cambio-planilla-equ-ju.php&mensaje=$mensaje");
 ?>