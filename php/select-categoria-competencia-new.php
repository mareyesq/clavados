...
if ($individual==1)
	$qry.=" AND individual=1";

if ($pareja==1)
	$qry.=" AND individual=0";
...
$qry.=" ORDER BY c.cod_categoria";
$ejecutar_qry=$conex->query($qry);
$num_regs=$ejecutar_qry->num_rows;
if ($num_regs==0)
	echo "<option value=''>no hay categorias definidas</option>";
else{
	while($reg=$ejecutar_qry->fetch_assoc()){
		$de=$reg["edad_desde"];
		$ha=$reg["edad_hasta"];
		$cod_categoria=$reg["cod_categoria"];
		$nombre_categoria=$cod_categoria."-".utf8_encode($reg["categoria"])." (de ".$de;
		if ($ha==99)
		...
		if(isset($categoria_sel)){
			if ($cod_categoria==$categoria_sel)
				echo " selected";
		}
